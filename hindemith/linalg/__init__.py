    ...
    def __call__(self, *args):
        ...
        for arg in args:
            if isinstance(arg, np.ndarray):
                if output is None:
                    output = np.zeros_like(arg)
                    out_buf, evt = cl.buffer_from_ndarray(self.queue, output, blocking=True)
                buf, evt = cl.buffer_from_ndarray(self.queue, arg, blocking=True)
                evt.wait()
        ...
        self._c_function(processed[0], processed[1], out_buf, self.queue,
                         self.kernel)
        buf, evt = cl.buffer_to_ndarray(self.queue, out_buf, output, blocking=True)
