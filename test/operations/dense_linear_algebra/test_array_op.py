...
class TestArrayOps(unittest.TestCase):
    ...
    def test_simple_array_add(self):
        self._check(ArrayAdd(backend='ocl'), ArrayAdd(backend='python'))

    def test_simple_array_sub(self):
        self._check(ArraySub(backend='ocl'), ArraySub(backend='python'))

    def test_simple_array_mul(self):
        self._check(ArrayMul(backend='ocl'), ArrayMul(backend='python'))

    def test_simple_array_div(self):
        self._check(ArrayDiv(backend='ocl'), ArrayDiv(backend='python'))
