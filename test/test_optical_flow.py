...
class TestOpticalFlow(unittest.TestCase):
    def test_optical_flow(self):
        ...
        for i in range(0, num_pyr):
            hm_du, hm_dv = hs_solver_jacobi(im1_data=pyr1[i], im2_data=pyr2[i],
                                            u=hm_u, v=hm_v, zero=zero, one=one,
                                            lam2=lam2, num_iter=num_iter)
            py_hm_du, py_hm_dv = py_hs_solver_jacobi(im1_data=pyr1[i], im2_data=pyr2[i],
                                            u=py_hm_u, v=py_hm_v, zero=zero, one=one,
                                            lam2=lam2, num_iter=num_iter)
            hm_u, hm_v = update_uv(u=hm_u, v=hm_v, du=hm_du, dv=hm_dv, two=two,
                                   w=sizes[i + 1][0], h=sizes[i+1][1])
            py_hm_u, py_hm_v = py_update_uv(u=py_hm_u, v=py_hm_v, du=py_hm_du, dv=py_hm_dv, two=two,
                                   w=sizes[i + 1][0], h=sizes[i+1][1])
